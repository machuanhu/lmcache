...
class NCCLManager:
    ...
    def __init__(self):
        ...
        self.device = int(os.getenv("CUDA_VISIBLE_DEVICES", "0").split(",")[0])
    ...
    def send_gpu_data(self, memory_obj: MemoryObj, target_rank: int) -> bool:
        ...
        try:
            ...
            if memory_obj.tensor.device.type != "cuda":
                ...
                memory_obj.tensor = memory_obj.tensor.cuda(self.device)
    ...
    def recv_gpu_data(self, source_rank: int, shape: torch.Size, dtype: torch.dtype, storage_manager=None) -> Optional[MemoryObj]:
        ...
        try:
            ...
            if storage_manager is not None:
                ...
                logger.info(f"存储管理器分配内存耗时: {t_allocate - t_start:.6f} 秒")
                if memory_obj is None:
                    logger.error("存储管理器分配内存失败")
                    return None
            else:
                # 如果没有存储管理器，回退到直接分配GPU内存
...
def get_nccl_manager() -> NCCLManager:
    """获取全局NCCL管理器实例"""
